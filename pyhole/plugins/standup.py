...
#
    ...
    def take(self, params=None, **kwargs):
        ...
        if params:
            ...
            if reservation is None:
                ...
                res = "%s takification complete" % cluster_id
        self.irc.reply(res)
    ...
    def ret(self, params=None, **kwargs):
        ...
        if params:
            force = params.endswith('force')
            print force
            if force:
                params = re.sub(r'force$', '', params)
            ...
            if reservation is None:
                ...
                if user == reservation['name'] or force:
                    ...
                    self.write_clusters(clusters)
                    res = "Forced to give up %s." if force else "%s releasified."
                    res = res % cluster_id
                else:
    ...
    def cluster(self, params=None, **kwargs):
        ...
        avail = lambda x: 'available' if not x else 'reserved (%s)' % res.get('name', 'some jerk')
    ...
    @plugin.hook_add_command('soup')
    def alias_soup(self, params=None, **kwargs):
        '''Alias of cluster'''
        return self.cluster(params, **kwargs)

    @plugin.hook_add_command('reservations')
    def alias_reservations(self, params=None, **kwargs):
        '''Alias of cluster'''
        return self.cluster(params, **kwargs)
    ...
    @plugin.hook_add_command('fact')
    def fact(self, params=None, **kw):
        if not self.facts:
            self.facts = self.load_facts()

        fact = random.choice(self.facts)
        self.facts.remove(fact)  # Don't tell this joke for a while.

        if not fact:
            fact = "Fact not found"
        self.irc.reply(fact)

    @plugin.hook_add_command('space')
    def space(self, params=None, **kw):
        if not self.spaces:
            self.spaces = self.load_spaces()

        space = random.choice(self.spaces)
        self.spaces.remove(space)  # Don't tell this joke for a while.

        if not space:
            space = "SPAAAAAACE"
        self.irc.reply(space)
